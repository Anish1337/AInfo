...
<h2>
  ...
  <p>
  Classification is achieved through a fully connected layer also known as multi-layered perceptron (MLP). This layer will be described shortly, but it is important to note that it only accepts one-dimensional data. Recall that what is sent after the pooling layer is in three dimensions (length, width, colors).
</p>
  <p>
  In order to combat this problem, the three-dimensional stacked feature map is then flattened into a one-dimensional array. This is known as the flattening step. Now the one-dimensional array is ready to be put into an multi-layered perceptron so that digits can be classified.
  </p>
<h2>
...
<p>
  ...
  is just a set number that measures how easy it is for a perceptron to reach a value of 1 or in biological terms how likely it is for a perceptron to fire. This will be useful later when training the MLP to recognize
...
<p>
  While the mathematics is very complicated, it is important to recognize that Gradient Descent refers to an algorithm where the minimum of the cost function graph (which would give 10
  ...
  perceptron equation . Ultimately, the adjustments of w and b values will yield the minimum cost, or in other words, the MLP (and therefore the entire CNN) will be trained to recognize handwritten digits accurately.
</p>
<h2>
